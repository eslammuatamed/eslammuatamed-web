# `app/pages` — التوجيه والتخطيطات والتدويل

> اقرأ [`PROJECT_GUIDE.md`](../../PROJECT_GUIDE.md) أولًا. يغطّي هذا الدليل الصفحات و`app/layouts/` و`app/middleware/` معًا (متلازمة التوجيه).

## المسؤولية

توجيه ملفّي (`Nuxt`) يقسم إلى العالمين: صفحات عامّة (SSR) وصفحات لوحة تحكّم (`ssr:false`). كل صفحة تجلب بياناتها عبر `useAsyncData(useApi())` وتضبط SEO عبر `useSeoMeta`.

## خريطة الملفّات

| الملف | الدور | العالم |
|---|---|---|
| `index.vue` | الرئيسية الكاملة (`FR-PUB-010…017`): بطل + تقنيات + مشاريع مميّزة + ملخّص خبرة + أحدث مقالات + توصيات + CTA تواصل، عبر `useSiteSettings` + `useHomeData` (جلب متوازٍ معزول لكل قسم)؛ `settings/site` تبعية صلبة (فشلها = حالة «الـ API غير متاح» `D13-1` — تعرض الآن علامة `UiBrandMark`)؛ بيانات `Person`+`WebSite` (`useSiteSchema`) وعنوان مستقلّ (`D22-4`). **أُعيد تصميم طبقتها البصرية في web-006** (التنسيق نفسه دون تغيير) | عام |
| `blog/index.vue` | قائمة المدوّنة المُرقَّمة (`GET /articles`)، حالة الصفحة في الـ URL query | عام |
| `blog/[slug].vue` | صفحة المقال (`GET /articles/{slug}`) + عرض `ContentProse` + `setI18nParams`. **سطح قراءة بعمود واحد موسّط** (015): العنوان والمقدّمة والنصّ كلّها محكومة بقياس واحد هو `--measure-prose`، ويُحرَّر `ContentProse` بـ `max-w-none` كي يملك العمود القياس وحده | عام |
| `experience.vue` | صفحة الخبرة المهنية (`FR-PUB-021`): جدول زمني عكسي زمنيًّا من `GET /experiences` عبر `useExperiences`، مع التقنيات من سجلّ المهارات (`D02-9`)؛ الحالات الثلاث عبر `UiRequestState`؛ `BreadcrumbList` فقط (لا `ProfilePage` — `D22-8`) | عام |
| `dashboard/login.vue` | تسجيل الدخول (`UForm` + zod)، بلا حارس، layout `auth` | لوحة (عميل) |
| `dashboard/index.vue` | overview مبدئية خلف الحارس (`middleware: 'auth'`, layout `dashboard`) | لوحة (عميل) |

التخطيطات (`app/layouts/`): `default.vue` (شِل عام: skip link + معالم + إدارة تركيز)، `dashboard.vue` (شِل عميل-فقط: topbar + خروج)، `auth.vue` (بطاقة موسّطة بلا chrome).

الحارس (`app/middleware/auth.ts`): per-page، عميل-فقط، تجديد صامت ثم توجيه للدخول — موصوف في [`app/stores/README.md`](../stores/README.md).

## التدفّقات

### القراءة العامّة (SSR)
```
useAsyncData(`key:${locale}`, () => api<Envelope<T>>('/path').then(r => r.data))
  → SSR: HTML مكتمل من أول رسمة (D13-2)
  → تبديل اللغة = تغيير مسار يعيد تركيب الصفحة بمفتاح جديد
```

### الترقيم (blog/index)
حالة الصفحة في `route.query.page` (قابلة للربط وواضحة لـ SEO، `D13-4`)؛ `useAsyncData` بـ `watch: [page]` يعيد الجلب عند التغيير؛ `UPagination` يربط الأرقام بـ query.

### المقال + سلاگات اللغات (blog/[slug])
بعد جلب المقال، `setI18nParams` يسجّل slug كل لغة كمعامل مسارها (F-P5): سلاگا EN وAR يختلفان، فبدون هذا يعيد المُبدِّل و`hreflang` استخدام slug اللغة الحالية فيعطّل (404). ثم `ContentProse` يعرض الجسم.

### المشاريع (005): الفهرس والتفصيل

- **`projects/index.vue`** — حالة المرشّح والصفحة في الـ URL (`D13-4`) فتكون العروض قابلة للمشاركة
  وتنجو من إعادة التحميل. الحالات مفصولة: هيكل تحميل أوّلي، وغلاف تحديث فوق محتوى قائم، وحالة فراغ
  **مُميَّزة** («لا دراسات حالة بعد» مقابل «لا مشاريع تستخدم هذه التقنية» مع إجراء إزالة التصفية)، وحالة خطأ
  مع إعادة محاولة متاحة.
  - **المرشّح صفّ أزرار `<button aria-pressed>` لا مكوّن قائمة منسدلة.** `USelect` كلّف **22.8 KB gz**
    على مسار محكوم بميزانية 250 KB مجمّدة (`doc 20 §1`)، وكان هو وحده سبب تجاوزها؛ وهو الحكم نفسه
    المسجَّل في `/contact` حين أبقت على `<select>` أصليّ. «كل التقنيات» **خيار حقيقي** في الصفّ لا
    عنصر نائب، فلا حاجة لزرّ إزالة منفصل داخل المرشّح. القيمة المُرسَلة هي `slug` (`D10-17`).
  - **تغيير المرشّح يستخدم `push` لا `replace`** — عكسٌ مقصود لقرار سابق كان مبرَّرًا بقائمة منسدلة
    تُراكم سجلًّا عند كل ضغطة سهم. الأزرار لا تمرّ بحالات وسيطة، والمكوّن لا يُطلق حدثًا عند إعادة ضغط
    الخيار النشط، فلا يُدفع رابط مطابق للحالي؛ وبذلك يُلغي زرّ الرجوع التصفية فعلًا.
  - **حجم الصفحة 6 يُرسَل صراحةً** (`PROJECTS_PER_PAGE`)، لا يُورَث من الافتراضي `12`: حجم الصفحة قرار
    تخطيط تملكه هذه الصفحة، وتركه ضمنيًّا يعني أنّ تغيير الافتراضي في الـ API يعيد تدفّق الصفحة صامتًا.
    و`page=1` يُحذف من الـ URL فيبقى لمحتوى واحد رابط واحد.
- **`projects/[slug].vue`** — 404 ليس نهائيًّا: عند 404 يُستشار جدول التحويلات مرّة واحدة (`D04-6`)،
  فإن وُجد `toPath` انتقلنا إليه بـ 301 بعد توطينه، وإلّا رُفع 404 الحقيقي. أيّ حالة غير 404 **تحتفظ
  برمزها** — تحويل 5xx إلى 404 يُزيل صفحة حيّة من الفهرسة.
  - **عمودان من `lg` (016)** — شبكة `grid` فيها عمود السرد وبطاقة `ProjectFacts`. البطاقة **أوّل**
    عنصر في الـ DOM لتُقرأ مباشرة بعد الملخّص على الشاشات الضيّقة، و`lg:order-last` وحده هو ما ينقلها
    إلى المسار الثاني ابتداءً من `lg`؛ ومسارات الشبكة تتبع اتجاه الكتابة فتنعكس في RTL دون CSS مرآتي.
    و**الالتصاق سلوك سطح المكتب فقط** (`lg:sticky`): بطاقة ملتصقة على شاشة صغيرة تسرق مساحة القراءة.
    الحلّ CSS خالص — لا JS ولا مراقب حجم.
  - **الحقائق تُذكر مرّة واحدة** — كانت السنة وقائمة التقنيات والروابط أشرطة منفصلة تحت الملخّص داخل
    `header`؛ صارت كلّها في البطاقة، فبقي في `header` العنوان الرئيسي والملخّص فقط، وبقي في الصفحة
    عنوان `h1` واحد بعد ضغط دعوة التواصل.
- **اللغة الفعّالة (`D06-6`)** — كلتا الصفحتين (ومثلهما `blog/[slug]`) تقرأ اللغة من **المسار** عبر
  `useRouteLocale()`، لا من حالة الواجهة، لأنّ `setup()` يعمل داخل نافذة تأجيل تثبيت اللغة في انتقال
  `D03-13`. التفصيل في `app/composables/README.md`.

### الخبرة المهنية (008): `/experience`

- **الترتيب ملك للـ API** — يعيد `GET /experiences` الترتيب `startDate desc` ثمّ `order asc`، وهو
  بعينه شرط «العكسي زمنيًّا» في `FR-PUB-021`؛ كما يرتّب تقنيات كل دور حسب `Skill.order`. **العميل لا
  يعيد الترتيب أبدًا** (القاعدة نفسها الموثّقة في `useProjects`): إعادة الترتيب تُلغي التحكّم الذي
  يملكه صاحب المحتوى في لوحة التحكّم.
- **لغة المحتوى من المسار** (`D06-6`): `useExperiences` يقرأ `useRouteLocale()`، ومفتاح
  `useAsyncData` يستخدم القيمة المُرسَلة نفسها، فلا تُخزَّن حمولة تحت مفتاح يسمّي لغة أخرى.
- **لا رجوع بين اللغات**: الـ API يُسقِط أي عنصر أو مهارة بلا ترجمة في اللغة المطلوبة (`D10-6`)،
  والواجهة تعرض ذلك كحالة فارغة حقيقية — لا تُعيد الطلب بلغة أخرى.
- **العناوين الواقعية تُعرض حرفيًّا**: التموضع التسويقي (`JavaScript Product Engineer`) شأن إعدادات
  الموقع، ولا يُعيد كتابة المسمّى الوظيفي التاريخي أبدًا.
- **الدلالات**: `<ol>` لأن الترتيب ذو معنى ويجب أن يبقى مفهومًا بلا CSS؛ التواريخ في عنصري `<time>`
  يحملان قيمة العقد؛ قائمة التقنيات `<ul>` موسومة باسم الدور؛ القضيب والعلامات مخفيّة عن التقنيات
  المساعدة.
- **الرأس (head) ملك للغة** (`D22-7`): الصفحة تكتب العنوان والوصف و`og:title`/`og:description` فقط.

### السيرة الذاتية (010): `/resume`

- **لا مصدر ثانٍ للحقيقة** (`FR-PUB-024`) — `useResumeData()` **يستدعي `useExperiences()` نفسه**
  الذي تستخدمه `/experience`، لا نسخة منه: الدالة ذاتها، والمفتاح ذاته، وترتيب الـ API ذاته. المهارات
  من `/skills`، والهوية من `/settings/site` على مساحة المفتاح `settings:site:{locale}` المشتركة مع
  شِل الموقع و`/about`، فتُحلّ الصفحة والتذييل من طلب واحد. **لا يوجد أي محتوى سيرة ذاتية في هذا
  المستودع.**
- **ملفّ الـ PDF واصِف، لا مصدر**: الصفحة **لا تُحلّل ملفّ الـ PDF أبدًا**. عندما يكون
  `resumeAsset` بقيمة `null` — وهي الحالة الحيّة الحالية — تُعرض ملاحظة صادقة مُعرّبة، ويبقى إجراء
  الطباعة متاحًا، ولا يُنشأ رابط معطوب، ولا يُعرض معرّف الأصل (المعرّف المجرّد إداري في العقد ولا يصل
  إلى الحمولة العامّة أصلًا)، ولا يُوجَّه المستخدم إلى `/contact` (شريحة لاحقة).
- **آلية التنزيل هي ترويسات الكائن**: وسم `<a href>` عادي إلى أصل الوسائط المطلق كما يخدمه العقد —
  بلا إعادة كتابة ولا إعادة بناء لمفتاح التخزين. سمة `download` في HTML **تُتجاهَل عبر الأصول
  المختلفة**، لذا فإن `Content-Disposition: attachment` المكتوبة كبيانات وصفية للكائن عند الرفع
  (`doc 19 §5`) هي المرجع، ويعمل الرابط بلا JavaScript.
- **البريد المهني فقط**: `professionalEmail` (بريد التوظيف والتواصل المهني)، ولا رجوع إلى
  `contactEmail`؛ وعند غيابه يُحذف السطر بالكامل. لا رقم هاتف ولا عنوان.
- **الطباعة** عبر `@media print` مقيّدة بـ `body:has(.resume-page)`: قواعد إخفاء الترويسة والتذييل
  تستهدف عناصر لا تملكها الصفحة، وأنماط الصفحة تبقى محقونة بعد التنقّل بعيدًا — بلا هذا القيد كانت
  طباعة `/about` بعد زيارة `/resume` ستفقد الترويسة والتذييل صامتةً. **لا يُفرض مقاس ورق**؛ الهوامش
  بوحدة `cm` فتصلح لـ A4 و US Letter معًا.
- **الدلالات**: `<h1>` واحد ثمّ `<h2>` لكل قسم؛ `<ol>` للأدوار لأن الترتيب ذو معنى؛ التواريخ في
  عنصري `<time datetime>` يحملان قيمة العقد؛ المهارات في `<dl>` بمجموعات الـ API وترتيبه.
- **الرأس (head) ملك للغة** (`D22-7`)؛ و`BreadcrumbList` فقط بلا `ProfilePage` ثانٍ (`D22-8` يسمّي
  `/resume` صراحةً)؛ ولا `og:image` خاصّ بالصفحة — فهي ترث البطاقة الاجتماعية المُلتزَمة التي
  يُصدرها `app.vue` على مستوى الموقع (`web-013` أغلقت `F-1`).

### حماية لوحة التحكّم
`login.vue` **بلا** حارس (كي يصلها الخارج) و`robots:noindex`؛ `index.vue` بـ `middleware:'auth'`. عزل SSR: `routeRules` في `nuxt.config.ts` تجعل `/dashboard/**` بـ `ssr:false`. ولا يوجد `/ar/dashboard/**`: كل صفحة لوحة تستدعي `defineI18nRoute(false)`، فمسارات اللوحة بلا بادئة لغة في اللغتين (D04-7)، واللوحة ثنائية اللغة عبر تفضيل لغة محفوظ لا عبر المسار (D02-15).

## العقود والثوابت

- كل قراءة عامّة مُوطَّنة (`?locale=` يضيفه `useApi`)؛ المفتاح يتضمّن اللغة.
- خطأ الجلب يُحوَّل عبر `articleErrorParams` (404 يبقى 404؛ غيره يحتفظ بحالته).
- صفحات لوحة التحكّم والخطأ `noindex`.

## الاختبارات

`components/layout/LocaleSwitcher.*.spec.ts` (تبديل اللغة)، و`Prose.spec.ts`. سلوك الصفحات مغطّى مبدئيًّا؛ اختبارات e2e (Playwright) مجدولة لمراحل لاحقة ([الوثيقة 18](../../../eslammuatamed-docs/docs/18-testing-strategy.md)).

## أخطاء شائعة (شرح لمطوّر مبتدئ)

- **جعل الحارس عامًّا** بدل per-page — يسرّب منطق لوحة التحكّم للعام (`D06-1`).
- **نسيان `setI18nParams`** في صفحة بـ slug مُترجَم — يكسر تبديل اللغة و`hreflang`.
- **قراءة لغة المحتوى من `useI18n().locale` في صفحة بـ slug لكل لغة** — يطلب سلاگ اللغة الجديدة
  باللغة القديمة أثناء التبديل فيظهر 404؛ استخدم `useRouteLocale()` (`D06-6`).
- **استدعاء `useI18n()`/`useSetI18nParams()` بعد الـ await** — استدعِها قبله لتعمل في سياق `Nuxt` صالح.
- **بناء صفحة dashboard بـ SSR** — يجب أن تبقى `ssr:false`.

## المرجع الرسمي وحالة التوافق

- [Nuxt pages/routing](https://nuxt.com/docs/guide/directory-structure/pages) · [Nuxt layouts](https://nuxt.com/docs/guide/directory-structure/layouts) · [`useAsyncData`](https://nuxt.com/docs/api/composables/use-async-data) · [`routeRules`](https://nuxt.com/docs/guide/concepts/rendering#route-rules) · [Nuxt i18n `setI18nParams`](https://i18n.nuxtjs.org/docs/composables/use-set-i18n-params) · [`useSeoMeta`](https://nuxt.com/docs/api/composables/use-seo-meta).

**حالة التوافق:** `Compatible`. التوجيه الملفّي، `definePageMeta`، `routeRules` لعزل SSR، و`useAsyncData` أنماط `Nuxt` رسمية حالية؛ و`setI18nParams`/`useSeoMeta` هي الـ APIs الحالية. **لا انحراف.**
