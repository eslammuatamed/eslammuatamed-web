...
### `format.ts`
`formatDate(value, locale)` عبر `Intl.DateTimeFormat` بنظام أرقام `latn` مثبّت حتى في `ar` (`D03-4`) — أرقام عربية غربية في اللغتين. ويضمّ كذلك عرض الخبرة: `formatMonthYear` و`formatExperiencePeriod`، و`impactBullets` الذي يفصل نصّ `impact` الماركداوني إلى بنود.

**لماذا `impactBullets` هنا تحديدًا؟** لأن **كلا** عرضَي الخبرة يستهلكانه — `ContentTimelineEntry` (الرئيسية و`/experience`) و`ResumeEntry` (`/resume`) — فلا يجوز أن يختلفا على تعريف «البند». كان في `utils/resume.ts`، فكان استيراده يجرّ وحدة مساعدات السيرة كاملة (تجميع المهارات، الروابط، أحجام الملفّات) إلى إغلاق كل مسار يعرض عنصر خبرة: قفزت `/` من 44 إلى 45 أصلًا ودخلت نطاق التحذير. و`format.ts` موجود أصلًا في تلك الإغلاقات الثلاثة (كلا المكوّنين يستدعي `formatMonthYear`)، فالتعريف المشترك هنا **بلا تكلفة**.
