...
describe('ContentWorkEntry', () => {
  // `row-glass` is what makes the WHOLE row read as the affordance (024, D03-15) — and critically, it
  // is what gives KEYBOARD users the same affordance as pointer users, because the class carries
  // `:focus-within` alongside `:hover`. Losing it silently reverts the row to "only the title changes
  // colour", which is a WCAG regression that no other assertion in this file would notice.
