...
// from the ROUTE (D06-6), so a locale switch cannot fetch the incoming page in the outgoing language.
export function useHomeData() {
  ...
  const projects = useAsyncData(
    ...
    () => api<Paginated<ProjectListItem>>('/projects', { locale: locale.value, query: { perPage: 6 } }).then(r => r.data),
    { watch: [locale] }
  )

  const skills = useAsyncData(
    () => `home:skills:${locale.value}`,
    () => api<Envelope<Skill[]>>('/skills', { locale: locale.value }).then(r => r.data),
    { watch: [locale] }
  )

  const experiences = useAsyncData(
    () => `home:experiences:${locale.value}`,
    () => api<Envelope<Experience[]>>('/experiences', { locale: locale.value }).then(r => r.data),
    { watch: [locale] }
  )

  const articles = useAsyncData(
    () => `home:articles:${locale.value}`,
    () => api<Paginated<ArticleListItem>>('/articles', { locale: locale.value, query: { perPage: 3 } }).then(r => r.data),
    { watch: [locale] }
  )

  const testimonials = useAsyncData(
    () => `home:testimonials:${locale.value}`,
    () => api<Envelope<Testimonial[]>>('/testimonials', { locale: locale.value }).then(r => r.data),
    { watch: [locale] }
  )
