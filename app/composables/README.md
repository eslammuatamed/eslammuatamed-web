...
| `useRequestState.ts` | النصف المُستدعِي من عقد `UiRequestState`: يشتقّ **تحميل أوّلي** (هيكل) مقابل **إعادة تحقّق فوق محتوى ظاهر** (طبقة)، و`show` لأقسام الرئيسية التي تُخفي نفسها وهي فارغة (`NFR-DEGRADE`) — قاعدة واحدة كانت مكتوبة حرفيًّا في **تسعة** مواضع (أقسام `home/` الخمسة وصفحات `about`/`experience`/`projects`/`blog`)، فصارت تعريفًا واحدًا لا يمكن أن تتفرّق نسخه |
