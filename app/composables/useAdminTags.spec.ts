...
function reset(rows: unknown[] = [{ id: 'tag-1' }]) {
  holder.calls = []
  holder.rows = rows
  holder.status = 0
  holder.parkNext = false
...
describe('the Tags collection read', () => {
  it('requests the production page contract and exposes server metadata', async () => {
    reset([{ id: 'tag-1' }, { id: 'tag-2' }])
    const source = useAdminTags()
    await source.load({ page: 2 })
    expect(holder.calls).toEqual([{
      path: '/admin/tags', options: { locale: false, query: { page: 2, perPage: 12 } }
    }])
