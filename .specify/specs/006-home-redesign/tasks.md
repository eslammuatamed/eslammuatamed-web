...
- [ ] T001 API dev-seed expansion on `chore/006-home-seed` (off API `dev` `a964349`): expand
...
- [ ] T002 Docs governance on a docs branch: **D16-9** (doc 16 §5.x DoD + §10 decision + review note),
...
- [ ] T003 Clean-DB integration harness: external temp env → throwaway `eslammuatamed_test`; run
  `db:seed` then `db:seed:dev` twice (idempotent proof); real `.env` never touched.

## Phase 1: Setup — brand primitives & tokens (Web)

- [ ] T010 `components/ui/BrandMark.vue` — inline Monolith SVG (`M2,6 H6 V2 H14 V10 H10 V14 H2 Z`),
  `currentColor`, size prop, `aria-hidden`.
- [ ] T011 `components/ui/DatumLabel.vue` — locale-robust eyebrow (mono index/coordinate for
  numerals/Latin only + script-appropriate eyebrow word + optional action + hairline rule),
  caller-controlled heading level.
- [ ] T012 `components/ui/Section.vue` — `<section>`+`<UContainer>` rhythm wrapper; `variant`
  base|elevated; consistent `--space-section`; optional top datum rule.
- [ ] T013 [P] main.css — minimal surface/rule helpers via existing tokens only; no new colours;
...
- [ ] T020 `index.vue` composes the redesigned sequence with `UiSection` variants (base/elevated
...
- [ ] T030 Update `Hero.spec.ts`, `TechStack.spec.ts`, `FeaturedProjects.spec.ts`,
...
### Implementation
- [ ] T031 `Hero.vue` — identity composition (BrandMark + availability pill + display name + role
  ladder + primary/secondary CTA); CSS-first reveal, reduced-motion safe; LCP text/inline-SVG.
- [ ] T032 `TechStack.vue` → `UiSection`+`UiDatumLabel`; grouped, dots only.
- [ ] T033 `FeaturedProjects.vue` + `content/ProjectCard.vue` — datum header, 3-card grid, mono year,
  border/surface-step hover; preserve single-accessible-link.
- [ ] T034 `ExperienceSummary.vue` (elevated variant) + `content/ExperienceItem.vue` — mono period,
  current-first, logical timeline rail (RTL-safe).

## Phase 4: User Story 2 — Reach, orient, read (P2)

### Tests
- [ ] T040 Update `LatestArticles.spec.ts`, `Testimonials.spec.ts`, `TestimonialCard.spec.ts`,
  `ArticleCard` coverage to new DOM; add article-ordering assertion.
### Implementation
- [ ] T041 `LatestArticles.vue` + `content/ArticleCard.vue` — redesigned meta/title/excerpt cards
  (mono date + reading time), 3 newest, `/blog/{slug}`.
- [ ] T042 `Testimonials.vue` + `content/TestimonialCard.vue` — bounded linear grid, quote rail.
- [ ] T043 `ContactCta.vue` — closing datum frame + mark echo; form link + mailto fallback (D05-4).
- [ ] T044 Apply T001 seed to the integration DB; verify home renders 3 newest articles ordered +
  localized (SC-4 subset).

## Phase 5: User Story 3 — Governance & seed (P3)

- [ ] T050 Land T002 (docs) + confirm T001 idempotent on clean DB (T003) — the D16-9 dogfood proof.
- [ ] T051 Update `components/layout/Footer.vue` datum alignment (behaviour unchanged: scheme
  allowlist WD-5, locale-reactive availability WD-6 preserved).

## Phase 6: Polish & Cross-cutting

- [ ] T060 Full Web gates: typecheck, lint, test, build, `check:bundle`, `check:logical`,
  `git diff --check`.
- [ ] T061 A11y/RTL pass: one h1, heading order, focus ring ≥3:1, 44 px targets, 200 %/320 px
  reflow, `bdi` Latin-in-Arabic, reduced-motion; browser screenshots mobile/tablet/desktop × EN/AR.
- [ ] T062 Integration verification (SC-4): SSR content-complete both locales, JSON-LD, standalone
  title, hreflang, zero console/hydration errors, sections from seeded data.
- [ ] T063 API gates on seed branch: typecheck, lint, test, `contract:export` (+ e2e if feasible).

## Phase FINAL: Documentation & Handoff Gate (MANDATORY — doc 16 §5.1 / D16-8 + new D16-9; always last)

- [ ] T070 Arabic module READMEs updated for every changed module (`components/`, `composables/` if
  touched, `pages/`; api `prisma/` seed doc note).
- [ ] T071 SpecKit closed (spec/plan/tasks checked); feature-map adds `006-home-redesign`
  (status "🧪 Implemented on dev").
- [ ] T072 Central docs merged (D16-9/D09-15 + CONTRIBUTING ×2 + tasks-template ×2).
- [ ] T073 PROJECT_GUIDE + handoff memory + MEMORY.md updated (dev-complete, not prod-shipped).
- [ ] T074 Final report: exact files, commits, SHAs, tests, seed commands, decisions, limitations,
  remaining production blockers, next safe owner-authorized action.
- [ ] T075 **Documentation & Handoff Gate sign-off (D16-8/D16-9)** — all above satisfied; feature
