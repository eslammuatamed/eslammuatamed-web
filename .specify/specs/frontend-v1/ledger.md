...
| **Last source-touching commit** | **CONVENTION, binding on every phase from FE-2b on: a phase commits its source, its tests and this ledger in ONE commit,** so there is never a gap between the code and the record of it. The previous shape stamped a SHA that a later doc-only commit then invalidated. FE-2c must do the same or this row goes false again. Verify with `git log -1 --name-only`. |
