...
**T5.D — OD-26-9 RESOLVED (Option A): flat D20-11 model REPLACED in direction; numeric calibration PENDING owner approval (ledger §27). Not started: T5.B · T5.F · T5.G · T5.H.** Everything **local-only, nothing pushed**.
...
| **T5.D** | **Public-route JS recovery** | ⛔ **OD-26-9 RESOLVED = Option A — flat D20-11 model REPLACED in direction; numeric calibration PENDING owner approval. Ledger §26 (attribution) + §27 (calibration package).** Attribution COMPLETE; no remediation path clears the gate. All three §25 open measurements are now CLOSED, on the same stamped build `8067ec8` with **no rebuild** and **zero application bytes changed**. **(1) THREE-WAY PARTITION** (floor / partially-shared / route-exclusive), 5 controls PASS incl. an **exact 18/18 reconciliation identity** (`floor + partial + exclusive == gate total`, to the byte) and a **negative control**. ⚠ The family rule is an **EXPLICIT TABLE**, not `slice(3)` — `@nuxtjs/i18n` translates the blog slug, so a path heuristic yields 10 families and misfiles the blog post’s own page chunk as partially-shared; **the controls caught it** (§26.1). Buckets: floor **37 assets / 254,554 B** · partial **13 / 10,748 B** · exclusive **14 / 45,773 B**. ⚠ **§25.5’s shared-chunk hypothesis is REFUTED** — `/contact` and `/projects/{id}` share **573 B**, not a common chunk; the 360 B proximity was coincidence (§26.3). ⚠ **THE DECISIVE MEASUREMENT (§26.4):** with **every route-exclusive byte deleted**, **6 of 9 route families STILL FAIL D20-11** (`/projects` +6,046 · `/blog` +5,327 · `/` +3,927 · `/experience` +2,907 · `/about` +1,604 · `/projects/{id}` +479); the other 3 pass only by cutting **79–94 %** of their own page code — for `/contact` that is **removing the contact form**. ⚠ The **largest route-exclusive item on the whole public surface** (`embla-carousel` 55.5 % of a 13,591 B gz chunk, arriving via Nuxt UI’s `UCarousel` in `project/Gallery.vue:131`) leaves its route **479 B over when removed entirely** — so no above-fold/interaction-gating analysis can change the verdict and none was run. ⚠ `/contact`’s Nuxt UI chunks are **the form primitives it requires** (`UForm`/`UFormField`/`UInput`/`UTextarea`/`UButton`/`UContainer`/`UIcon`) — **no unused-import lever**. ⚠ **The partially-shared bucket is mostly APP-OWNED** (six chunks 100 % `(app)`) — do **not** describe non-floor route cost as “primarily unavoidable framework delivery”. ⚠ **SECOND COUNTERFACTUAL (§26.9, derived not asserted — a draft absolute “app-code reduction would not clear it on any route” was FALSE and corrected before commit):** deleting **all app-owned** bytes leaves **3 families PROVABLY UNREACHABLE** (`/contact` retains 9,652 B of `@nuxt/ui`-only form chunks; `/blog`+`/projects` each retain `BKk1jqLu.js` 2,447 B, reka-ui + `@nuxt/ui`, **zero** app bytes — each alone > the 1,446 B allowance), **1 UNDETERMINED** (`/projects/{id}`’s 13,591 B chunk is MIXED and gz is not decomposable per package), and **5 needing 44–86 %** cuts of their own app-owned delivery. ⚠ §26.4 (delete route-EXCLUSIVE) and §26.9 (delete ALL app-owned) answer **different questions**; neither subsumes the other. **(2) `(generated)` 64,952 B TRACED** (§26.7), control-reconciled to the byte across 32 modules, and it is **NOT one owner**: Nuxt UI generated theme **26,196 B (40.3 %)** · `@nuxt/icon` client bundle 12,824 (19.7 %) · Nuxt route/router manifests 11,898 (18.3 %) · app config 5,740 (8.8 %) · build-tooling runtime 3,729 (5.7 %) · Nuxt glue 4,565 (7.0 %). ⚠ **Lever REFUTED:** `UProgress`/`UToast`/`UToaster`/`UChip`/`UAvatar` are in **zero** app `.vue` files but are **reachable by construction** — `@nuxt/ui` `App.vue:13` statically imports `UToaster` → `UToast` → `UProgress`, and `UApp` wraps the app. Second instance of the `reference-unreferenced-css-var-is-wrong-test` lesson. **(3) i18n PRECOMPILATION ANSWERED **NO**, from primary sources** (§26.8). `bundle.runtimeOnly` and `bundle.dropMessageCompiler` **are** supported options (`module.d.mts:227`; `module.mjs:2451-2452` aliases `vue-i18n.runtime`), **but their documented precondition is unmet**: the docs require messages **pre-compiled** and say dropping the compiler *“will not work”* when resources load dynamically. This app’s messages are **not precompiled** — the module’s own `readStaticResource` returns **`JSON.stringify(messages)`** and comments *“these resources are not precompiled”* (`module.mjs:1531`), and a probe string resolves **only** to `.output/server/chunks/raw/8311b79d.mjs`, **no client asset**. So the full `vue-i18n.mjs` (32,049 B) + `@intlify/message-compiler` (36,905 B) are the **upstream-owned cost of the separately-delivered JSON model OD-26-7 deliberately adopted**. **`@nuxtjs/i18n` 10.6.0 PRESERVED.** ⚠ `experimental.prerenderMessages` is **not** a workaround — it changes **where** JSON is served, not whether it is compiled, and is budget-neutral (`.json` is not collected by the `'js'` asset pass). ⚠ **NO PROBE BUILD RUN** — every candidate is closed by documented incompatibility or reachability, so nothing remains a gz control build could cost (same reasoning that withdrew §24.17’s probe). **ESCALATION (OD-26-9):** the trigger is met **not** because the floor exceeds D20-11 (it does not — 254,554 < 256,000, and that must not be misstated) but on the question that matters after attribution: **the floor consumes 99.4 % of D20-11, leaving 1,446 B gz — smaller than the minimum viable page in this app (simplest route = 2,599 B)**. Options **A** recalibrate on shared-floor + per-route allowance (**no numbers proposed**) · **B** durable attributed acceptance · **C** direct further remediation anyway. ⚠ **This escalation does NOT cite and does NOT depend on D20-30** — its non-precedent clause is respected; structural similarity is not a citation. ⚠ **No budget proposed/raised/re-baselined/waived/weakened; the gate still exits 1 on 18 public routes and must never be reported green.** ⚠ **T6.6 unchanged** — public routes are the **second independent red** on the same gate, so resolving T5.D by decision does **not** make it pass and promotion stays mechanically impossible **OD-26-9 OUTCOME (§27):** owner ruled the flat total-route model **structurally unfit** and approved a **replacement direction** — ⚠ explicitly **NOT** a weakening: durable attributed acceptance was **REJECTED** for the public routes (visitor-facing/SEO/Lighthouse-sensitive); they must end Campaign 026 **GREEN**, not under a permanent red exception. ⚠ **NOTHING IMPLEMENTED — every §27 number is PROPOSED, NOT APPROVED**; `route-assets.mjs` and doc 20 are **unmodified**. **Shared premise of all candidates: gate the per-route DELTA above the floor, never the total** (a total re-charges framework growth to every page — the original defect). ⚠ **Candidate D (4th gate on the partially-shared bucket) REJECTED ON EVIDENCE — 70.0 % of that bucket sits at `fams=2`, one route from reclassifying, vs the floor boundary moving only 296 B (0.12 %)**; so A/B/C gate `total − floor` as ONE quantity. **RECOMMENDED = Candidate B:** floor cap + **3 tiers by page FUNCTION** (T1 content 7 KiB · T2 collection 12 KiB · T3 interactive-subsystem 18 KiB) — 4 numbers, **no per-route table**, 9/9 pass, lightest route may grow **2.8×** vs 7.1× under one generic allowance. ⚠ **House ×1.15 idiom REJECTED for the floor cap** (38,310 B headroom = **2.4× the worst route’s whole allowance**); derived instead from measured framework movement (i18n 10.6.0 = +1,946 B/route) → **257 KiB recommended**, and that multiplier is flagged as **the number most needing owner judgement**. ⚠ **Bounds:** ≥ **9,652 B** or `/contact` is permanently red · ≥ **15,876 B** for all routes to pass today. ⚠ **The new effective per-route total is HIGHER than the old 250 KB — that is the CORRECTION, not a relaxation** (the old number was arithmetically unsatisfiable by any page, §26.4); every application-facing guard gets **stricter**. ⚠ **Route-set sensitivity:** adding a governed route can eject a floor asset and trip every delta gate at once → **FREEZE the floor asset list** (D20-12/D20-29 frozen idiom). ⚠ **Derived independently of D20-30 — do not merge the two models.** **i18n + Unhead conclusions PRESERVED.** ⛔ **T6.6 unchanged; promotion still mechanically impossible.** ⏳ **AWAITING OWNER:** model choice, floor-cap multiplier, tier caps, frozen-floor mitigation |
...
| **T6.6** | ⚠ **CARRIED PRODUCTION BLOCKER — do not solve inside Phase 5.** `/dashboard/messages` remains **mechanically red** under the current `size:routes` gate at **337,421 B gz** against the unchanged **327,680 B** hard ceiling (**+9,741 B**), gate **exits 1**. **D20-30's narrow owner-attributed architectural acceptance does NOT make the gate pass** — it accepts the residual as attributed while changing no number, and doc 20 §5 admits no passing state above the ceiling. **Mechanism:** `deploy.yml`'s `verify` runs `size:routes` with **no `continue-on-error`**, and `deploy` **`needs: verify`**. **Consequence:** Production promotion (T7.3/T7.4) is **mechanically impossible**, not merely unauthorized, while this gate is unchanged. ⚠ **Second, independent red on the same gate:** T5.D's public routes are over D20-11 — clearing Dashboard alone does **not** unblock promotion, and clearing public routes alone does not either. **Resolution DEFERRED** to the D11-8 post-campaign Dashboard UI/UX pass and its budget-model recalibration. **Forbidden here and in Phase 5:** no `continue-on-error`, no removing `size:routes` from `verify`, no de-listing a route, no waiver state, no interim numeric ceiling, no raising a number to fit. Ledger §24.21.7-ter | ⛔ **BLOCKER — carried into Phase 6, gates Phase 7** |
