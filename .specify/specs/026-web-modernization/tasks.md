...
⚠ **STATUS: IN PROGRESS — NOT CLOSED.** Checkpoint at Web `5aa3f2f` / Docs `736ef1d`, both
**local-only, nothing pushed**. Done: **T5.A · T5.C (CSS budget GREEN) · T5.P**. Open: **T5.E
(OD-26-8)** and **T5.D**, both at owner decisions. **Not started: T5.B · T5.F · T5.G · T5.H.**
⚠ **Next action: answer OD-26-8 before starting any new task group** — its outcome decides whether
T5.E needs implementing. Full zero-trust resume state: ledger **§24.17**.
...
| **T5.E** | **`/dashboard/messages` recovery** | ⏸ **EVIDENCE COMPLETE → OD-26-8 OPEN. Not implemented.** Gap **+9,741 B** over the 327,680 B ceiling (**+30,221 B** over the 307,200 B target — two thresholds, not one). ⚠ **Lazy boundaries are ZERO real recovery here**: `/dashboard/**` is `ssr:false` and the closure follows STATIC imports only, so a `Lazy*` cuts the MEASURED number while the browser downloads the same first-paint bytes — gate-gaming. Simple/supported remedies **exhausted**. Both remaining candidates are **policy-gated by D11-8**, so neither was implemented: **A** `UTable`→plain `<table>` (one consumer, only `:data`/`:columns`/`:aria-label` used; UTable stack = **54.5 %** of a 40,191 gz chunk) · **B** zod→`zod/mini` across **all four** consumers (18,776 gz chunk, 93.1 % zod; all-or-nothing). ⚠ Accepting the residual is an equally valid third option. Ledger §24.16 |
