...
| T3.2 | Apply the upgrade in the primary worktree; capture typecheck under **both** methods | **DONE** — applied at `58c0cc0`/`86f5610`. Typecheck run under `nuxt prepare`-generated types after a clean install **and** after a full `nuxt build`: **7 errors both ways**, so the historical 18-vs-12 spread does not recur. Four tree states measured in one directory (§14.2) |
| T3.3 | Fix TS family (a) | **ROOT-CAUSED, NOT PATCHED — ledger §14.7.** `nuxt-schema-org` switches vendored builds by `unhead` major; the v3 vendor tightened `Record<string, any>` → `Record<string, unknown>`, closing the `any` hole the whole-graph getter was passing through. **Both obvious fixes refuted on evidence:** `computed()` is snapshotted by `toValue()` during SSR setup and would silently emit `knowsAbout: []` on `/`; field-level getters are rejected by `DeepResolvableProperties` for exactly the async-fed **array** fields. Every remaining option is a boundary assertion — the 'no suppression' line. Deferred pending **OD-26-5** rather than refactoring 7 SSR-critical sites against an unsettled stack |
