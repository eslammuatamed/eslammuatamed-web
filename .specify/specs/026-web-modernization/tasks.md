...
**T5.D OPEN (floor measured, two measurements outstanding — §25). Not started: T5.B · T5.F · T5.G · T5.H.** Everything **local-only, nothing pushed**.
...
⚠ **T5.D IS NOW STARTED and is OPEN at an evidence boundary — ledger §25.** The public-route floor
was measured on one basis at build `8067ec8`: **the true intersection across all 18 public routes is
254,554 B gz = 99.4 % of D20-11 (256,000 B)**, leaving **1,446 B gz for ALL route-specific delivery**,
which **no route achieves**. **The binding constraint is the shared floor, not route code.**
⚠ **The escalation trigger is NOT met** (254,554 < 256,000) — **no owner decision was opened and no
escalation package drafted**, and none may be drafted until the two open measurements below are done.
⚠ **§24.6's arithmetic is CORRECTED** (renderedLength compared against gz); §24.12's CSS conclusion
survives unchanged. **Next session's first measurement: the route-unique app/vendor partition**
(§25.5), then **the i18n precompilation question from primary docs** (§25.6). **`@nuxtjs/i18n`
10.6.0 is preserved.** Zero application bytes changed.
...
| **T5.D** | **Public-route JS recovery** | ⏸ **OPEN — floor MEASURED; NOT tractable, NOT escalated. Two measurements outstanding.** 18/18 over D20-11 (+1.1 KB best … +14.1 KB worst). ⚠ The **lightest** route is still over while its app-owned bytes sit at less than half the cap, and a **−1,695 B CSS win moved routes only ~0.4–0.5 KB** — clearing the worst would need ~28× the entire Phase 5 CSS recovery from a sheet already 919 B under its cap. **D20-11 is not reachable through CSS** (§24.12 — unaffected by the correction below, and in fact strengthened: CSS is not in the JS floor at all). ⚠ **§24.6's arithmetic is CORRECTED by ledger §25** — it compared an app **renderedLength** figure against a **gz** route total, which the gate's own header forbids; its "zeroing app code would not clear the lightest route" claim is **false as stated**. ⚠ **MEASURED at build `8067ec8` (HEAD-valid: only `tasks.md` differs, zero app sources), instrument positively controlled 18/18 against the gate to the byte:** the **true intersection across all 18 public routes is 254,554 B gz = 99.4 % of the 256,000 B D20-11**, leaving **1,446 B gz for ALL route-specific delivery** — and **no route achieves it** (lightest 2,599 B route-unique, +1,153; heaviest `/contact` 15,876 B, +14,430). **The binding constraint is the shared floor, not route code.** ⚠ **NOT tractable and NOT escalated:** the escalation trigger ("the supported floor itself exceeds the budget") is **NOT met** — 254,554 < 256,000 — so no owner decision was opened and no package drafted. ⚠ **Two OPEN measurements before any verdict:** (1) **the route-unique app/vendor partition** — `total − floor` is **not** app-owned, and `/contact`'s validation library and `/projects/{id}`'s 10,397 B `MediaBrowser.vue` static import suggest vendor dominates it, which would be a **permitted** priority-#5/#6 lever, so §24.6's "app code was never the constraint" must **not** be carried forward as settled; (2) **the i18n precompilation question** — the full `vue-i18n.mjs` + `@intlify/message-compiler` (36,905 B renderedLength) sit in every route's floor, but 10.6.0's separately-delivered JSON messages may genuinely require the compiler, in which case it is OD-26-7's adopted cost and `runtimeOnly` is unavailable without a forbidden revert. **`@nuxtjs/i18n` 10.6.0 is preserved either way.** ⚠ Floor package figures are **renderedLength apportionment ESTIMATES**, never measured gz — no candidate may be adopted on them without a gz control build. Zero application bytes changed. Ledger §25 |
