...
⚠ **STATUS: NOT CLOSED — every task is DONE except the hard-exit CONTRACT, which T6.6 prevents.**
Done: **T5.A · T5.B · T5.C (CSS budget GREEN) · T5.D (D20-31, public routes GREEN) · T5.F · T5.G ·
T5.P**, and **T5.H hosted verification is COMPLETE**. **T5.E is PAUSED** (accepted-as-attributed
under D20-30, not recovered).

⚠ **Phase 5 does NOT close, and the reason is a single requirement.** T5.H's contract includes
`/dashboard/messages` ≤ 320 KB gz; hosted CI measures **337466 B (329.6 KB gz), +9786 B over the
unchanged 327,680 B ceiling, gate exit 1**. That is **T6.6**, and it is preserved unchanged — not
gamed, waived, de-listed or re-numbered. **Every other T5.H requirement passed on the exact pushed
SHA.** ⚠ Do NOT read "all Phase 5 tasks done" as "Phase 5 closed": the hard-exit contract, not the
task list, is what closes a phase. **Phase 6 was NOT started.**

⚠ **ONE push has now occurred — the FIRST of Phase 5 and the ONLY one of that session** (owner-
authorized, T5.H evidence only): Web **`da83531`** → `origin/026-p3-nuxt` for PR **#66**, hosted run
**32036256592**. ⚠ **Not the first push of Campaign 026** — Phase 3 pushed this same branch three
times for the same CI-evidence purpose (`a6159f6`, `4685911`, `cf6910c`, each with a `pull_request`
run). Phase 5 held no push authorization before then and used none. ⚠ **`origin/dev` (`ced8490`) and
`origin/main` (`52e77d9`) did NOT move**; nothing was merged, promoted or deployed; PR #66 remains
**draft**; the private Docs repo stays **local-only**.
...
| **T5.H** | **Phase 5 hard exit gate** (supersedes old T5.12) — CSS ≤ 30,000 B · all governed public-route hard budgets · `/dashboard/messages` ≤ 320 KB gz · app-owned frozen caps · font budget · JS/bundle gates · no EN/AR/RTL regression · no a11y regression · all code/test gates green · hosted CI + hosted E2E + BOTH Lighthouse profiles green on the exact final Phase 5 SHA. **If the modernized stack cannot fit through legitimate cleanup, STOP for an owner decision — do not raise, re-baseline or weaken any budget** | ⚠ **HOSTED VERIFICATION COMPLETE — CONTRACT NOT SATISFIED. Evidence collected, Phase 5 stays OPEN at an owner-decision boundary.** Verified SHA **`da83531`**, run **32036256592**, PR **#66** — the ONE authorized push of the campaign (`cf6910c`→`da83531`, no force; `dev`/`main` unmoved). **Preflight demanded no change**, so the candidate is the T5.G checkpoint: `api:types` a fixed point, lint/typecheck/typecheck:e2e exit 0, unit 1481/1481. ⚠ **The hosted run measures `refs/pull/66/merge` (`7958ea08`), and that was PROVEN harmless by tree-hash identity, not assumed** — `dev` is an ancestor of `da83531` and the merge tree `914f4de` **equals** `da83531`'s tree, so every figure is attributable byte-for-byte. **GREEN, hosted:** E2E · Accessibility **412/412 passed, zero failed/flaky, `retries: 0`** (⚠ the T5.G local 409/412 `contact-a11y` class **did not reproduce** — hosted was named the discriminator in advance and it discriminated; **no rerun was issued** and **the SHA did not change**, so no second push authorization is needed) · **BOTH** Lighthouse profiles, **0 failed assertions**, 16/16 governed URLs each, **a11y/best-practices/SEO = 100.0 median on all 32 readings**, worst perf 82.0 (`/ar` mobile, ≥ 60) · lint · typecheck · typecheck:e2e · unit **1481/1481** · build · `api:types` · **CSS `size` 29.08 kB gz / 30 kB — green on hosted CI for the FIRST time this campaign** · `size:routes` public surface **18/18 ✓** with floor **254599 B / 257.0 KB ✓** and app-owned **18/18 + 8/8 PASS**. ⛔ **THE ONE FAILURE:** `size:routes` **exit 1** (⚠ exit **1**, not 2 — a genuine breach in a valid build, not a measurement failure) on **`/dashboard/messages` = 337466 B gz (329.6 KB), +9786 B over the unchanged 327,680 B hard ceiling** — **sole ✗**, = **T6.6**, preserved unchanged. ⚠ **`check:bundle` and `check:logical` were SKIPPED and are NOT claimed as CI-green**: they sit after `size:routes` in the same `bash -e` job, so **hosted CI structurally cannot reach them while T6.6 stands** — green **locally only**, and that is a consequence of T6.6, not a second blocker. ⚠ **Hosted reads +6 B vs local** (337466 vs 337460; floor 254599 vs 254593) — uniform build-environment noise; **the hosted figure is the authoritative one** and the delta is stated so it is never misread as drift. ⚠ **3 governed Lighthouse readings pass THINLY** and are recorded as thin: Arabic fonts `/ar` **132080 B of 133120 (99.2 %, 1040 B headroom)**, LCP mobile `/ar/about` **3904 ms of 4000 (97.6 %)**, CLS mobile `/ar/about` **0.0487 of 0.05 (97.4 %)**. ⚠ **3 dashboard routes sit in the D20-24 warning band and still print `NO ACCEPTED BASELINE RECORDED`** (the 5-of-8 gap, re-confirmed hosted). **Ledger §31** — including §31.10's three governed options for making the accepted Dashboard condition mechanically releasable, and why a public/dashboard gate split is a **waiver in disguise** and is not offered. ⚠ **NET: EVIDENCE COMPLETE — BLOCKED ON T6.6, awaiting an owner decision. Phase 5 stays OPEN; Phase 6 NOT started.** |
