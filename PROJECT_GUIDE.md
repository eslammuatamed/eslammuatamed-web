...
### 7.1 ملكية العقد وتبنّيه والتحقيق في تاريخه

- **المصدر الرسمي:** `openapi.json` المُصدَّر والمُلتزَم من `eslammuatamed-api` هو العقد الوحيد بين
  الـ API والويب. `openapi/openapi.json` هنا نسخة مزامنة (vendored copy) فقط، وليست مكانًا لتصميم
  العقد أو تعديله استقلاليًا.
- **الفصل بين العقد والاختبار:** لا تُضاف أمثلة API أو بيانات mock أو fixtures لـ Prism أو schemas
  خاصة بالاختبار إلى النسخة المورّدة. تبقى fixtures المملوكة للواجهة خارج الأثر، مثل
  `scripts/e2e/`; وPrism يقرأ العقد الملتزَم فقط.
- **الأنواع المولّدة:** بعد مزامنة النسخة المورّدة شغّل `npm run api:types`. ملف
  `app/types/api.d.ts` ناتج مولّد من تلك النسخة ولا يُحرَّر يدويًّا.

عند مزامنة العقد، استخدم التسلسل التالي وسجّل في commit/PR التبنّي: SHA مصدر الـ backend، ونتيجة
`sha256sum` لملف backend `openapi.json`، ونتيجة `sha256sum openapi/openapi.json` بعد النسخ:

1. قبل أي حكم على `origin/dev` أو `origin/main` أو تاريخ/نسب فرع backend، نفّذ
   `git fetch origin --prune`. لا تُستنتج الملكية أو نقطة الأساس من refs محلية قديمة.
2. استخدم SHA من فرع التنفيذ الموثّق بعد ذلك التحديث؛ ففروع الوثائق أو النسخ الاحتياطية أو التشغيل
   ليست نقطة أساس لتنفيذ المنتج ما لم توثّقها المستودعات صراحةً.
3. صدّر العقد من المصدر الخلفي، انسخه كما هو، ثم ولّد الأنواع والتكيّف المطلوبين في commit الذرّي.
   لا تُصلح drift بتحرير JSON أو الأنواع المولّدة انتقائيًا.
4. إن كشف التبنّي drift دلاليًا واسعًا وغير متوقع لا يتعلق بالتغيير الخلفي المقصود، **توقّف** قبل
   استبدال العقد. حقّق في provenance أولًا بدل الكتابة فوق النسخة أو انتقاء تعديلات يدوية.

وعند التحقق من إصلاح تاريخي، لا يكفي سؤال «هل SHA معيّن موجود؟». افحص السلوك الحالي ومخرجات العقد
المولّدة، وابحث في التاريخ بالملف/الرسالة/المحتوى عن commits مكافئة؛ هوية commit ليست هوية الإصلاح
الدلالية.
